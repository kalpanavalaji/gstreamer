...
/*
//swapping of consecutive nodes
void swap_node()
{
int ele;

    if (Head == NULL || Head->Nlink == NULL)
        return;
 
    
    NODE *Prev = Head, *Temp = Head->Nlink;
    Head = Temp;  
 
   
    while (1)
    {
        NODE *T_Temp = Temp->Nlink;

        Temp->Nlink = Prev;

        if (T_Temp == NULL || T_Temp->Nlink == NULL)
        {
            Prev->Nlink = T_Temp;
            break;
        }
 	
        Prev->Nlink = T_Temp->Nlink;
